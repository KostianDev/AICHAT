...
/**
 * Calculates adaptive epsilon for DBSCAN based on k-nearest neighbor distances.
 * Uses the k-distance graph heuristic from the original DBSCAN paper.
 * 
 * @param points Input color points
 * @param n Number of points
 ...
 * 
 * Optimized implementation based on "DBSCAN Revisited, Revisited" (Schubert et al., 2017).
 * Uses a 3D grid for O(1) average-case neighbor lookups instead of O(n) brute force.
 * For well-distributed color data with small eps, achieves near-linear time complexity.
 * 
 * @param points Input color points
 * @param n Number of points
 * @param eps Epsilon neighborhood radius (recommended: 10-50 for RGB 0-255)
 ...
 * Noise points (label == -1) are excluded from centroid calculation.
 * 
 * @param points Input color points
 * @param n Number of points
